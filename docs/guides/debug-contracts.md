...
3. For a render-plan failure, compute the initial value in Go or use a portable
   ...
   widget may use the nearest `use client` module boundary; confirm the
   compiler reports its exact downgrade (`triggerCode` / `triggerConstruct`) and
   consider wrapping only that subtree in `ClientOnly`. Rank remaining
   downgrades with `gobeyond report portability`. Explicit `ClientOnly` remains
   available with an optional fallback. `<Columns>` provides portable
   multi-column content or gallery layout when that presentation fits.
4. For hydration warnings, compare Go’s no-JS document with React’s first
   render. Check props, condition ordering, list keys, dynamic indexes
   (missing/OOB → empty like React `undefined`), URLs, whitespace, IDs, and
   client-only fallbacks. An empty boundary must be empty in Go and on the
