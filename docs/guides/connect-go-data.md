...
The Go loader declares the serializable payload React needs. Code generation
then writes `page.schema.ts`, which gives `page.tsx` its React-facing inferred
type and produces the route registry. The build projects route-owned Go source
...
func Page(ctx *gb.PageContext) (gb.PageResult[Props], error) {
  ...
  return gb.OK(Props{Product: product}, metadataFor(product)), nil
}
```
...
To reuse loaded props across requests, declare origin ISR in `Config` beside
`Props`. GoBeyond emits the equivalent generated `page.schema.ts`:
...
`Config.Revalidate` controls origin props reuse; set `gb.CachePolicy` on the
loader for HTTP caching and keep both windows aligned deliberately (for example
`gb.PublicRevalidate(60*time.Second, …)` beside `Revalidate: 60`).
