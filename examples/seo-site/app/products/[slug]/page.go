...
import (
	...
	contract "github.com/Origens-Dev/gobeyond/examples/seo-site/internal/gobeyondgen/contracts/routes/r_products_slug_3e2e8eb9"
...
func Page(ctx *gb.PageContext, params Params) (gbruntime.LoadedPage, error) {
	...
	if params.Slug != "trail-pack" {
		return gbruntime.LoadedPage{Kind: gb.ResultNotFound, Status: http.StatusNotFound, Cache: gb.CachePolicy{Mode: gb.CachePrivateNoStore}, Metadata: gb.Metadata{Lang: "en", Title: "Product not found"}, Props: contract.Props{Slug: params.Slug, Name: "Product not found", Image: "/missing.jpg", Currency: "USD", Availability: string(PropsAvailabilityOutOfStock)}}, nil
	}
	canonical := origin + "/products/trail-pack"
	props := contract.Props{Slug: "trail-pack", Name: "Trail Pack", Description: "A durable pack for portable React.", Canonical: canonical, Image: origin + "/images/trail-pack.svg", ImageAlt: "Blue Trail Pack", Price: 129, PriceLabel: "$129.00", Currency: "USD", Availability: string(PropsAvailabilityInStock)}
