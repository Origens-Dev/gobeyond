...
func generateClientEntry(website string, modules []compilerRouteModules, manifestRoutes []project.Route) (browserBuildInput, error) {
	...
	source.WriteString("import { bootstrapAsync } from '@gobeyond/react/browser'\n\n")
...
func writeClientRouteModule(path, routeDirectory, website string, route compilerRouteModules) error {
	...
	source.WriteString("\nconst Route: ComponentType<any> = (props: any) => ")
	expression := "createElement(Page as ComponentType<any>, props)"
	for index := len(route.LayoutFiles) - 1; index >= 0; index-- {
		expression = fmt.Sprintf("createElement(Layout%d as ComponentType<any>, props, %s)", index, expression)
	}
